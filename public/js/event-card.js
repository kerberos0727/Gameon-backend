...
function initialize(user) {
            ...
            getGeoLocation(address)
                .then(location => {
                    if (location) {
                        ...
                        const request = selectedRow ? makeRequestWithToken('/api/cards/event/' + selectedRow, 'PATCH', data) : makeRequestWithToken('/api/cards/event', 'POST', data)
                        if (request) {
                            request.done(() => {
                        ...
                        $.toast({
                            heading: 'Error',
                            text: 'Google Location api service isn\'t working.',
                            icon: 'error'
                        })
                    }
                })
    ...
    if (user.role === 'leader') {
        columns = [...columns.slice(0, 1), ...columns.slice(2)]
    } else if (user.role === 'team') {
        columns = [...columns.slice(0, 1), ...columns.slice(3)]
    }
...
function selectRow(rowId, pwFlag = false) {
    ...
    if (rowId) {
        if (resultData) {
            ...
            const request = makeRequestWithToken('/api/cards/event/' + rowId, 'GET')
            if (request) {
                request.done(row => {
                    initValues({
                        name: row.name,
...
function deleteRow(rowId) {
    const request = makeRequestWithToken('/api/cards/event/' + rowId, 'DELETE')
    if (request) {
        request.done(user => {
            const resultTbl = $('table#result').DataTable()
