...
function initialize(user) {
    ...
    $('#image').change(function() {
        ...
        // console.log('image file', $('input[type=file]')[0].files[0]);
    ...
    $('form.add-card').validate({
        ...
        submitHandler() {
            ...
            const request = selectedRow ? makeRequestWithToken('/api/cards/trial/' + selectedRow, 'PATCH', data) : makeRequestWithToken('/api/cards/trial', 'POST', data)
            if (request) {
                request.done(() => {
...
function deleteRow(rowId) {
    const request = makeRequestWithToken('/api/cards/trial/' + rowId, 'DELETE')
    if (request) {
        request.done(user => {
            const resultTbl = $('table#result').DataTable()
